...
`timescale 1ps / 1ps
  `define CFG_FUNC_VERSION         23_Oct_2018   //     Check length on reads
...
`endif
  ...
  , input          cfg_bad_rd                    // HW469861 Gate rd_q latch
...
always @(posedge(clock))
  if (reset_q == 1'b1) 
    ...
    reg_oinfo_cmd_valid_q <= 1'b0;                    // Clear cmd_valid after Descriptor table has processed current command
  else
